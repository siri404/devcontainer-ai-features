...
This repository contains a _collection_ of Features. Currently it includes the `gemini-cli` Feature that provides access to Google's Gemini AI models from within your dev container.
...
For example, the `gemini-cli` feature provides options for specifying the version and API key.
...
```jsonc
{
    ...
    "options": {
        ...
        "apiKey": {
            "type": "string",
            "default": "",
            "description": "Optional: Google API key for Gemini (can also be set via environment variable GEMINI_API_KEY)"
        }
    }
}
```
...
```jsonc
{
    "image": "mcr.microsoft.com/devcontainers/base:ubuntu",
    "features": {
     "ghcr.io/my-org/private-features/gemini-cli:1": {
            "version": "latest"
        }
    },
    "customizations": {
